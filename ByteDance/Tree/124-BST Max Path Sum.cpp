...
public:
    ...
    int dfs(TreeNode* root) {
        if (!root) return 0;
        int left = max(dfs(root->left), 0); // consider negative numbers
        int right = max(dfs(root->right), 0);
        int val = left + right + root->val;  // take both left and right subtree
        maxSum = max(maxSum, val);
        return max(left, right) + root->val; // choose either left or right subtree
    }
};
